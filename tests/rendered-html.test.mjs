...
test("server-renders the Hanpan mobile app shell", async () => {
  await withDevServer(async (baseUrl, response) => {
    ...
    assert.equal((dealerHtml.match(/class="dock-icon"/g) ?? []).length, 2);
    ...
    assert.match(dealerHudStyles, /grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/);
    assert.match(dealerHudStyles, /gap:\s*1cqw\s+\.45cqw/);
    assert.match(dealerHudStyles, /\.seat\.highest:not\(\.me\):not\(\.seller\)\s*\{[\s\S]*box-shadow:\s*none\s*!important/);
    assert.match(dealerHudStyles, /left:\s*76\.4cqw/);
    assert.match(dealerHudStyles, /width:\s*51\.65cqw/);
...
test("keeps the requested game set and removes excluded modes", async () => {
  ...
  assert.match(page, /room\.surpriseEnabled === false/);
  ...
  assert.match(surprise, /waitingSurpriseState/);
...
test("ships 177 unique trivia questions and one fixed image catalog", async () => {
  ...
  assert.equal(trivia.length, 177);
  assert.equal(new Set(trivia.map((item) => item.question)).size, 177);
