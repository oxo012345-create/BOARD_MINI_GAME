...
export default function Home() {
  ...
  if (room.view === "briefing" && currentGame) {
    ...
    return <main className={`app-shell briefing-shell ${currentGame.id === "gem-heist" ? "gem-briefing-shell" : ""}`}>
      ...
      <div className="sticky-action">{isHost ? <button className="button primary xl" disabled={hostActionLocked || (currentGame.id === "gem-heist" && !gemPlayerCountValid) || (currentGame.id === "maze-courier" && !mazePlayerCountValid) || (currentGame.id === "double-dealers" && !dealerPlayerCountValid) || (currentGame.id === "cash-n-guns" && !cashNGunsPlayerCountValid)} onClick={() => void startGame()}>{currentGame.id === "gem-heist" ? "사건 시작" : currentGame.id === "maze-courier" ? "배달 대결 시작" : currentGame.id === "double-dealers" ? "경매장 입장" : currentGame.id === "cash-n-guns" ? "캐시 앤 건즈 시작" : "게임 시작"}</button> : <div className="waiting"><span className="pulse" />방장이 게임을 시작하기를 기다리는 중</div>}</div>
      {commonOverlays}
    </main>;
  }
