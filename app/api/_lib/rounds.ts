...
export type GameRound = {
  ...
  siegeWar?: import("./siege-war").SiegeWarState;
};
...
GAME_INFO["siege-war"] = { title: "2대2 3라인 공성전", briefing: "두 명씩 편을 나눠 왼쪽·중앙·오른쪽 세 라인에서 병사, 궁수, 공성추를 쏟아붓습니다. 상대 성문 세 개 중 하나만 무너뜨리면 즉시 승리하고, 남은 시간 1분부터는 총공세로 소환 대기가 사라집니다.", category: "solo" };
...
function makeRoundCandidate(id: string, players: Player[], liarMode: "normal" | "dumb" = "normal", specialRoles = false, gemDifficulty: GemDifficulty = "normal"): GameRound | null {
  ...
  if (id === "siege-war") return { ...base, prompt: "세 라인을 밀어 성문을 무너뜨리세요" };
  if (id === "telestration") {
