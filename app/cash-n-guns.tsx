...
export function CashNGunsGame({ code, meId, state, isHost, debugMode = false, busy, onAction, onReplay, onLobby, onLeave, overlays }: {
  ...
  const playFx = (name: FxName) => {
    ...
    if (name === "button") tone(210, .035, .014);
    if (name === "select") { tone(480, .06, .025); tone(720, .08, .018, "square", .04); }
    if (name === "confirm") { tone(360, .06, .025); tone(620, .12, .03, "square", .05); }
    if (name === "click") tone(95, .07, .04, "triangle");
    if (name === "gun") { noise(.22, .16); tone(72, .26, .12, "sawtooth"); }
    if (name === "hit") { tone(58, .3, .09, "sawtooth", .08); noise(.16, .07, .08); }
  ...
  useEffect(() => { setReservations(state.my.lootReservationIds ?? []); }, [state.my.lootReservationIds]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (state.phase === "resolve" && state.roundOutcome) {
      ...
      shots.slice(0, 5).forEach((shot, index) => window.setTimeout(() => playFx(shot.result === "bang" ? "gun" : "click"), index * 150));
      if (state.roundOutcome.woundedIds.length) window.setTimeout(() => playFx("hit"), Math.min(600, shots.length * 150));
      if (vibrationOn && navigator.vibrate) navigator.vibrate(state.roundOutcome.woundedIds.length ? [70, 45, 130] : 35);
    } else if (state.phase === "loot") playFx("loot"); else playFx("phase");
  ...
  const selectPlayer = (playerId: string) => {
    if (!isSelectable(playerId)) return; playFx("select");
  ...
  const saveReservations = (next: string[]) => { if (!state.my.canReserveLoot || busy) return; setReservations(next); playFx("select"); act({ action: "cash-n-guns-reserve-loot", reservationIds: next }); };
  const toggleReservation = (lootId: string) => {
    const index = reservations.indexOf(lootId);
    if (index < 0) { saveReservations([...reservations, lootId]); return; }
    if (index === 0) { saveReservations(reservations.slice(1)); return; }
    const next = [...reservations]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; saveReservations(next);
  };
  ...
  return <main className="cng2-shell" onPointerDownCapture={(event) => { if ((event.target as Element).closest("button")) playFx("button"); }}><div className="cng2-game-frame">
    ...
    <section className={`cng2-scene phase-${state.phase} ${naturalAim || naturalGodfather ? "selecting-player" : ""}`} aria-label={`CASH AND GUNS ${PHASE_LABEL[state.phase]}`}><div className="cng2-environment" aria-hidden="true" /><div className="cng2-phase-chip"><span>{PHASE_LABEL[state.phase]}</span><i>{alivePlayers.length}명 생존</i></div>
      <svg className="cng2-aim-lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><defs><marker id="cng2-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" /></marker></defs>{allAimLinesVisible && state.players.flatMap((player) => { const from = SEAT_COORDS[seatByPlayer[player.id]]; const to = player.aimTargetId ? SEAT_COORDS[seatByPlayer[player.aimTargetId]] : undefined; if (!from || !to || !player.alive) return []; return <g key={`${player.id}-${player.aimTargetId}`}><line x1={from.x} y1={from.y} x2={to.x} y2={to.y} markerEnd="url(#cng2-arrow)" className={player.id === meId ? "mine confirmed" : "confirmed"} /><circle className={player.id === meId ? "target-ring mine" : "target-ring"} cx={to.x} cy={to.y} r="2.5" /></g>; })}{activeTargetSeat && mySeat && <g className={naturalGodfather ? "command-line" : "active-line"}><line x1={mySeat.x} y1={mySeat.y} x2={activeTargetSeat.x} y2={activeTargetSeat.y} markerEnd="url(#cng2-arrow)" /><circle className="target-ring active" cx={activeTargetSeat.x} cy={activeTargetSeat.y} r="3.6" /><circle className="target-dot" cx={activeTargetSeat.x} cy={activeTargetSeat.y} r="1" /></g>}</svg>
      <div className="cng2-loot-rail" aria-label="이번 라운드 전리품">{state.currentLoot.map((card) => { const taken = state.lootTakenIds.includes(card.id); const rank = reservations.indexOf(card.id) + 1; const detail = lootDetail(card).split(" · "); return <button type="button" key={card.id} className={`${taken ? "taken" : ""} ${rank ? "reserved" : ""}`} disabled={taken || !state.my.canReserveLoot || busy} onClick={() => toggleReservation(card.id)} aria-label={`${card.label}${rank ? ` 예약 ${rank}순위` : " 예약하기"}`}><LootSprite card={card} /><span className="loot-info"><b>{detail[0]}</b><small>{detail.slice(1).join(" · ")}</small></span>{rank > 0 && <em>{rank}</em>}</button>; })}{state.newGodfatherAvailable && (() => { const rank = reservations.indexOf("godfather-token") + 1; return <button type="button" className={`token ${rank ? "reserved" : ""}`} disabled={!state.my.canReserveLoot || busy} onClick={() => toggleReservation("godfather-token")} aria-label={`대부 토큰${rank ? ` 예약 ${rank}순위` : " 예약하기"}`}><LootSprite card={{ kind: "token" }} /><span className="loot-info"><b>대부 토큰</b><small>다음 라운드 대부</small></span>{rank > 0 && <em>{rank}</em>}</button>; })()}</div>
      {state.my.canReserveLoot && <div className="cng2-reservation-bar"><span>{reservations.length ? `예약 ${reservations.length}개 · 재터치하면 순위↑` : "전리품을 눌러 예약"}</span><button type="button" disabled={!reservations.length || busy} onClick={() => { setReservations([]); act({ action: "cash-n-guns-reset-reservation" }); }}>예약 초기화</button></div>}
