...
const FRONTIER_SOUNDS: Record<FrontierSound, string> = {
  ...
  trade: "/frontier-beans/audio/trade-success.ogg",
...
function useFrontierSound() {
  ...
  const setMuted = useCallback((muted: boolean) => {
    ...
    if (!bgm) return;
    bgm.muted = muted;
    if (!muted && unlockedRef.current) void bgm.play().catch(() => undefined);
  }, []);
  useEffect(() => () => { bgmRef.current?.pause(); bgmRef.current = undefined; }, []);
...
function PlayerPanel({ player, index, active, tradeable, onTrade }: { player: FrontierBeanClientPlayer; index: number; active: boolean; tradeable: boolean; onTrade?: () => void }) {
  return <button type="button" className={`fb3-player ${active ? "active" : ""} ${tradeable ? "tradeable" : ""}`} disabled={!onTrade} onClick={onTrade}>
    <span className="fb3-player-head"><img src={farmerImage(index)} alt="" draggable={false}/><span><b>{player.name}</b><small className="fb3-player-metrics"><span className="fb3-metric coin"><i aria-hidden="true"/>{player.coins}</span><span className="fb3-metric cards"><i aria-hidden="true"/>{player.handCount}</span></small></span><span className="fb3-player-status">{player.receivedCount > 0 && <em className="pending">심기 {player.receivedCount}</em>}{tradeable && <em>거래</em>}{active && <em>턴</em>}</span></span>
...
export function FrontierBeansGame({ code, meId, state, isHost, busy, onAction, onLobby, onLeave, overlays }: Props) {
  ...
  const [drag, setDrag] = useState<DragState>(); const [dragOverField, setDragOverField] = useState<number>(); const [plantFlash, setPlantFlash] = useState<number>(); const [eventFx, setEventFx] = useState<"harvest" | "plant" | "trade">(); const [tradeClosing, setTradeClosing] = useState(false);
  ...
  const { play: playSound, unlock: unlockSound, setMuted: setSoundMuted } = useFrontierSound();
  ...
  useEffect(() => {
    ...
    const settle = window.setTimeout(updateHandOverflow, 180);
    const observer = hand && typeof ResizeObserver !== "undefined" ? new ResizeObserver(updateHandOverflow) : undefined;
